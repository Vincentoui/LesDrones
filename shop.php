        ...
        <div class="container wow animate__animated animate__slideInLeft">
            ...
            <div class="row g-1">
				...
				<?php
					...
					$sql = "SELECT id, name, quantity, mass, price, image_url FROM food";
					$result = $conn->query($sql);

					if ($result->num_rows > 0) {
					  // récupération et affichage des articles un par un
					  while($row = $result->fetch_assoc()) {

						echo "<div class='col-md-3'><div class='card p-3 shop_articles'><div class='text-center'> <img src='" . $row["image_url"] . "' width='200'> </div>";
						echo "<div class='product-details'> <span class='font-weight-bold d-block'>" . $row["price"] . "€</span> <span>" . $row["name"] . "</span><div class='buttons d-flex flex-row'>";
						echo "<div class='cart'><i class='fa fa-shopping-cart'><a href='article_info.php?article=". $row["id"] ."'><img class='img-fluid' src='images/info_icon.png'></a></i></div> <button class='btn btn-primary cart-button btn-block'><span class='dot'>1</span>Ajouter</button></div>";
						echo "<div class='weight'> <small>" . $row["quantity"] . " piece " . $row["mass"] . "kg</small> </div>";
						echo "</div></div></div>";
					  }
					} else {
